...
import WeeklyCompresSPH


class ParticleTypes:
    """Enum-like class for particle material types."""
    RIGID = 0
    FLUID = 1
...
@ti.data_oriented
class ParticleField:
    """Helper class to manage particle-related fields."""
    def __init__(self, dim: int, total_particle_num: int):
        self.dim = dim
        self.position = ti.Vector.field(dim, dtype=ti.f32, shape=total_particle_num)
        self.color = ti.Vector.field(3, dtype=ti.f32, shape=total_particle_num)
        self.material = ti.field(dtype=ti.i32, shape=total_particle_num)
        self.tmp_cnt = ti.field(ti.i32, shape=())

    @ti.kernel
    def copy_fluid_data(self, src: ti.template(), dst: ti.template(), material_fluid: int):
        """Generic method to copy fluid-only data between fields."""
        self.tmp_cnt[None] = 0
        for i in src:
            if self.material[i] == material_fluid:
                dst[ti.atomic_add(self.tmp_cnt[None], 1)] = src[i]


class ParticleCounter:
    """Helper class to manage particle counting operations."""
    def __init__(self, dim: int, particle_diameter: float):
        self.dim = dim
        self.particle_diameter = particle_diameter

    def count_particles(self, start: np.ndarray, end: np.ndarray) -> int:
        """
        Compute the number of particles that can fit in a volume.

        Args:
            start: Starting coordinates of volume
            end: Ending coordinates of volume

        Returns:
            int: Total number of particles that fit in volume
        """
        return np.prod([len(np.arange(start[i], end[i], self.particle_diameter))
                       for i in range(self.dim)])


class FluidManager:
    """Helper class to manage fluid-specific operations."""
    def __init__(self, particle_field: ParticleField, total_fluid_particles: int):
        self.particle_field = particle_field
        self.fluid_only_color = ti.Vector.field(3, dtype=ti.f32,
                                              shape=total_fluid_particles)
        self.fluid_only_position = ti.Vector.field(particle_field.dim, dtype=ti.f32,
                                                 shape=total_fluid_particles)

    def update_fluid_fields(self, material_fluid: int):
        """Update both position and color fields for fluid particles."""
        self.particle_field.copy_fluid_data(
            self.particle_field.position,
            self.fluid_only_position,
            material_fluid
        )
        self.particle_field.copy_fluid_data(
            self.particle_field.color,
            self.fluid_only_color,
            material_fluid
        )
...
class ParticleSystem:
    def __init__(self, simulation_config):
        """Initialize the particle system with the given configuration."""
        # Basic configuration
        self.simulation_config = simulation_config
        self.config = self.simulation_config['Configuration']
        self.rigidBodiesConfig = self.simulation_config['RigidBodies']
        self.fluidBlocksConfig = self.simulation_config['FluidBlocks']
        
        # Initialize mesh-related lists
        self.mesh_vertices = []
        self.mesh_indices = []
        
        # Initialize domain parameters
        self._init_domain_parameters()
        
        # Calculate total particle numbers first
        self._calculate_particle_numbers()
        
        # Initialize helper classes after particle numbers are known
        self._init_helper_classes()

    def _init_domain_parameters(self):
        """Initialize domain and particle-related parameters"""
        ...
        self.padding = self.support_length
        self.grid_num = np.ceil(self.domain_size / self.grid_size).astype(np.int32)

    def _calculate_particle_numbers(self):
        """Pre-calculate total number of particles"""
        # Calculate fluid particles
        ...
        # Calculate rigid body particles
        ...
        self.total_particle_num = self.total_fluid_particle_num + self.total_rigid_particle_num

    def _init_helper_classes(self):
        """Initialize helper classes after particle numbers are known"""
        ...
        self.particle_counter = ParticleCounter(self.dim, self.particle_diameter)
        self.particle_field = ParticleField(self.dim, self.total_particle_num)
        self.fluid_manager = FluidManager(self.particle_field, self.total_fluid_particle_num)
    ...
    def free_memory_allocation(self):
        # Delete simulation configuration objects
        ...
        # Delete sorting and grid-related objects
    ...
    def compute_particle_count(self, start, end):
        """Compute the number of particles that can fit in a volume defined by start and end points."""
        return np.prod([len(np.arange(start[i], end[i], self.particle_diameter))
                       for i in range(self.dim)])

    @ti.kernel
    def update_fluid_position_info(self):
        """Update fluid_only_position with positions of fluid particles."""
        self.tmp_cnt[None] = 0
    ...
    @ti.kernel
    def update_fluid_positions(self):
        """Update fluid particle positions in the fluid-only position field."""
        self.fluid_manager.update_fluid_fields(self.material_fluid)

    @ti.kernel
    def update_fluid_color_info(self):
        """Update fluid_only_color with colors of fluid particles."""
        self.tmp_cnt[None] = 0
        for i in self.color:
            if self.material[i] == self.material_fluid:
    ...
    def update_fluid_colors(self):
        """Update fluid particle colors in the fluid-only color field."""
        self.fluid_manager.update_fluid_fields(self.material_fluid)
    ...
    def update_mesh_info(self, vertices: ti.types.ndarray(), indices: ti.types.ndarray(),
        ...
        """Copy mesh vertices and indices from numpy arrays to Taichi fields."""
    ...
    def get_mesh_info(self, mesh):
        """Convert mesh vertices and faces to Taichi fields and store them."""
    ...
    def build_solver(self):
        return WeeklyCompresSPH.WCSPHSolver(self)
